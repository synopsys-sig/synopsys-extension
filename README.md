...
| `NETWORK_AIR_GAP` | If the `NETWORK_AIR_GAP` is set to true, Synopsys Security Scan Extension for Azure DevOps will not download the Synopsys Bridge but instead use the pre-configured Synopsys Bridge. If the Synopsys Bridge is configured at a specific location, provide the path through `SYNOPSYS_BRIDGE_INSTALL_DIRECTORY`, <br/><br/>The Synopsys Security Scan Extension will look for the Synopsys Bridge from `SYNOPSYS_BRIDGE_INSTALL_DIRECTORY` path; otherwise, it will look for the Synopsys Bridge in the default path. | Optional     |
